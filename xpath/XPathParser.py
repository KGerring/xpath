...
class Parser:
    ...
    def parse(self, text):
        ...
        while 1:
            ...
            if yyn == YYFLAG:
                ...
                if (
                    0 <= yystate <= YYLAST
                    and yycheck[yystate] == state_stack[state_ptr]
                ):
            ...
            if yychar == YYEMPTY:
                ...
                while lexer_last < lexer_end:
                    ...
                    try:
                        ...
                        matched = reduce(
                            lambda result, item: item[1] is None and result or item,
                            list(match.groupdict().items()),
                            (None, None),
                        )
    ...
    def report_error(self, state, line, column, lval):
        ...
        msg = "parse error at line %d, column %d: matched '%s'" % (line, column, lval)
...
def relativeLocationPath2(self, __stack, __ptr):
    ...
    """
    __val = ParsedRelativeLocationPath.ParsedRelativeLocationPath(
...
def step2(self, __stack, __ptr):
    ...
    """
    __val = ParsedStep.ParsedStep(
        __stack[__ptr + 1], __stack[__ptr + 2], __stack[__ptr + 3]
    )
    return __val
...
def abbreviatedAbsoluteLocationPath1(self, __stack, __ptr):
    ...
    """
    __val = ParsedAbbreviatedAbsoluteLocationPath.ParsedAbbreviatedAbsoluteLocationPath(
        __stack[__ptr + 2]
    )
    return __val
...
def abbreviatedRelativeLocationPath1(self, __stack, __ptr):
    ...
    """
    __val = ParsedAbbreviatedRelativeLocationPath.ParsedAbbreviatedRelativeLocationPath(
        __stack[__ptr + 1], __stack[__ptr + 3]
    )
    return __val
...
def pathExpr3(self, __stack, __ptr):
    ...
    """
    __val = ParsedExpr.ParsedPathExpr(
        __stack[__ptr + 2], __stack[__ptr + 1], __stack[__ptr + 3]
    )
    return __val
...
def equalityExpr2(self, __stack, __ptr):
    ...
    """
    __val = ParsedExpr.ParsedEqualityExpr(
        __stack[__ptr + 2], __stack[__ptr + 1], __stack[__ptr + 3]
    )
    return __val
...
def relationalExpr2(self, __stack, __ptr):
    ...
    """
    ops = {"<": 0, ">": 2, "<=": 1, ">=": 3}
    __val = ParsedExpr.ParsedRelationalExpr(
        ops[__stack[__ptr + 2]], __stack[__ptr + 1], __stack[__ptr + 3]
    )
    return __val
...
def multiplicativeExpr2(self, __stack, __ptr):
    ...
    """
    ops = {"*": 0, "div": 1, "mod": 2}
    __val = ParsedExpr.ParsedMultiplicativeExpr(
        ops[__stack[__ptr + 2]], __stack[__ptr + 1], __stack[__ptr + 3]
    )
    return __val
...
patterns = {
    INITIAL: re.compile(
        "(?P<p00>\\)|\\])|(?P<p01>::)|(?P<p02>\\.\\.)|(?P<p03>//)|(?P<p04>=|!=)|(?P<p05><=|<|>=|>)|(?P<p06>(node|text|comment|processing-instruction)(?=\\s*\\())|(?P<p07>[a-zA-Z_][a-zA-Z0-9\\.\\-_]*(?=\\s*::))|(?P<p08>('[^']*')|(\"[^\"]*\"))|(?P<p09>(\\d+(\\.(\\d+)?)?)|(\\.\\d+))|(?P<p10>\\$([a-zA-Z_][a-zA-Z0-9\\.\\-_]*:)?[a-zA-Z_][a-zA-Z0-9\\.\\-_]*)|(?P<p11>([a-zA-Z_][a-zA-Z0-9\\.\\-_]*:)?[a-zA-Z_][a-zA-Z0-9\\.\\-_]*(?=\\s*\\())|(?P<p12>([a-zA-Z_][a-zA-Z0-9\\.\\-_]*:\\*)|(([a-zA-Z_][a-zA-Z0-9\\.\\-_]*:)?[a-zA-Z_][a-zA-Z0-9\\.\\-_]*)|\\*)|(?P<p13>[\\t\\n\\r\\s]+)|(?P<p14>.)",
        re.M,
    ),
    OPERATOR: re.compile(
        "(?P<p15>or)|(?P<p16>and)|(?P<p17>\\*|mod|div)|(?P<p18>[\\t\\n\\r\\s]+)|(?P<p19>.)",
        re.M,
    ),
}
pattern_actions = {
