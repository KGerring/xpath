...
def CoreNumberValue(object):
    ...
    if type(object) in [type(1), type(2.3), type(4)]:
        return 1, object
...
def _strInstance(object):
    ...
    if hasattr(object, "nodeType"):
        ...
        if node_type == Node.ELEMENT_NODE:
            ...
            text_elem_children = [
                x
                for x in object.childNodes
                if x.nodeType
                in [Node.TEXT_NODE, Node.ELEMENT_NODE, Node.CDATA_SECTION_NODE]
            ]
            return reduce(
                lambda x, y: CoreStringValue(x)[1] + CoreStringValue(y)[1],
                text_elem_children,
                "",
            )
