...
class XPath(Parser):
    ...
    def AdditiveExprs(self, v):
        ...
        if _token_ == "PLUS":
            ...
            MultiplicativeExpr = self.MultiplicativeExpr()
            AdditiveExprs = self.AdditiveExprs(
                self.nop(self.PLUS, v, MultiplicativeExpr)
            )
            return AdditiveExprs
        elif _token_ == "'-'":
            self._scan("'-'")
            MultiplicativeExpr = self.MultiplicativeExpr()
            AdditiveExprs = self.AdditiveExprs(
                self.nop(self.MINUS, v, MultiplicativeExpr)
            )
            return AdditiveExprs
        elif _token_ in [
            "'<'",
    ...
    def MultiplicativeExprs(self, v):
        ...
        if _token_ == "MultiplyOperator":
            ...
            UnaryExpr = self.UnaryExpr()
            MultiplicativeExprs = self.MultiplicativeExprs(
                self.nop(self.TIMES, v, UnaryExpr)
            )
            return MultiplicativeExprs
        elif _token_ == "'div'":
            self._scan("'div'")
    ...
    def ChildOrAttributeAxisSpecifier(self):
        ...
        if _token_ in ["'@'", "NodeType", "STAR", "QName", "NCNameStar", "NCName"]:
            ...
            raise SyntaxError(
                self._pos, "Could not match ChildOrAttributeAxisSpecifier"
            )
...
if sys.hexversion > 0x2000000:
    ...
    def _get_type(match):
        type = val = None
        for t, v in list(match.groupdict().items()):
            if v is None:
                continue
            if val:
                raise SyntaxError(
                    pos, "ambiguity:%s could be %s or %s" % (val, type, t)
                )
            type = t
            val = v
        return type, val
